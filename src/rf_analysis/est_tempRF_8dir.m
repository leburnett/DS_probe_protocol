        ...
        for plot_n = 1:4
            ...
            for f_dt = -10:1:10
                ...
                for fc = 1:numel(frame_changes)
                    ...
                    v_data = av_resp(frame_changes(fc)+1:frame_changes(fc+1));
                    ...
                    % end 
                    f_norm(f_norm==0)=-1; % convert 0 to -1. 
...
rf_data_t = zeros(20, 192);

for f_dt = -10:1:10

    % rf_data_all = zeros(48, 192);

for plot_n = 3 %[1,3] % Just use 20 dps conditions %1:4
    ...
    for j = 1:8
        ...
        for fc = 1:numel(frame_changes)
            ...
            % If f_dt is -ve, showing frames shown before voltage (delay), if f_dt
            % is positive showing frames after voltage (predicting).
            f_data = pattern.Pats(:, :, av_frame(frame_changes(f_id)));

            % What frame was being shown during this time? 
            % f_data = patternPats(:, :, av_frame(frame_changes(fc)));
...
figure; imagesc(rf_data_t)
hold on
plot([0 192], [11 11], 'k', 'LineWidth', 1.2)
title(date_str)
